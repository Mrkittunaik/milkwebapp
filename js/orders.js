...
export async function placeRealOrder({ cart, address, lat, lng, locationAccuracy, couponCode, paymentStatus, paymentRef, paymentOrderId, slot }) {
  const items = Object.values(cart).map(item => ({ productId: item.id, qty: item.qty }));
  const order = await ordersApi.create({
    items, address, lat, lng, locationAccuracy, couponCode, paymentStatus, paymentRef, paymentOrderId, slot
  });
